...
function Content(props) {
   ...
   const [createdPdfs, setCreatedPdfs] = useState([])
   // const [update, setUpdate] = useState(0)

   const retrivesavedPdfs = async (userId) => {
      try {
         const response = await axios.post(`http://localhost:3500/userSavedPdfs`, {
            _id: userId
         });
   ...
   const retriveCreatedPdfs = async (userId) => {
      try {
         const response = await axios.post(`http://localhost:3500/userCreatedPdfs`, {
            _id: userId
         });
         setCreatedPdfs(response.data)
   ...
   useEffect(() => {
      retrivesavedPdfs(userId)
      retriveCreatedPdfs(userId)
   }, [reload])
   ...
   return (
      <div className='px-10 mt-8'>
         <div className='flex'>
            ...
            <Upload setReloading={setReloading}></Upload>
         </div>
         <div className='mt-5'>
            <span className='font-bold text-lg'>Appunti salvati:</span>
            <div>
               {(isLoadingSaved === false && savedPdfs.length !== 0) &&
                  savedPdfs.map((pdf, index) => (
                     <AnteprimaPdf key={index} pdf={pdf}><RemoveButton setReloading={setReloading} pdfId={pdf._id} type={"saved"}/></AnteprimaPdf>
                  ))
               }
               {isLoadingSaved === true &&
            ...
            <div>
               {(isLoadingCreated === false && createdPdfs.length !== 0) &&
                  createdPdfs.map((pdf, index) => (
                     <AnteprimaPdf key={index} pdf={pdf}><RemoveButton setReloading={setReloading} pdfId={pdf._id} type={"created"} /></AnteprimaPdf>
                  ))
               }
               ...
               {(isLoadingCreated === false && createdPdfs.length == 0) &&
                  <span>nessun contenuto caricato</span>
               }
            </div>
